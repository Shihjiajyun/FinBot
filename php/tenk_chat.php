...
$companyName = $ticker;
try {
    $stmt = $pdo->prepare("SELECT company_name FROM financial_analysis WHERE ticker = ? LIMIT 1");
    $stmt->execute([$ticker]);
    ...
    error_log("獲取公司名稱錯誤: " . $e->getMessage());
}
?>
...
<body>
            ...
            <div class="tenk-messages-container" id="messages-container">
                <div class="welcome-message">
                    ...
                    <div class="message-content">
                        ...
                        <p>我可以幫您分析 <?= htmlspecialchars($ticker) ?> 的<?= $isAllFiles ? '所有' : '指定' ?> 10-K 財報檔案。請提出您的問題：</p>
                        <div class="suggested-questions">
                            ...
                            <button class="suggested-btn" onclick="askSuggestedQuestion('財務表現和關鍵指標如何？')">
                                財務表現
                            </button>
    ...
    <script>
        ...
        function formatAnswer(answer) {
            ...
            // 使用markdown-it渲染
            let formattedAnswer = window.md.render(cleanAnswer);

            // 如果有圖表，添加圖表容器
            if (charts.length > 0) {
                charts.forEach((chartData, index) => {
                    const chartId = `chart-${Date.now()}-${index}`;
                    formattedAnswer += `<div class="chart-container"><canvas id="${chartId}"></canvas></div>`;
        ...
        function renderChart(canvasId, chartData) {
            ...
            try {
                ...
                canvas.parentElement.innerHTML = '<p style="color: #ff6b6b;">圖表渲染失敗</p>';
            }
        }
